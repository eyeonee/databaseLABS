...
class Model:
    ...
    def generate(self, table, count1, count2):
        try:
            ...
            if table == "coach":
                self.connection.commit()
                self.cursor.execute(f"""INSERT INTO public.coach
                                SELECT chr(floor(65 + random()*25)::int) || 
                ...
                self.connection.commit()
                self.cursor.execute(f"""INSERT INTO public.team
                ...
                self.connection.commit()
                self.cursor.execute(f"""INSERT INTO public.player
                                SELECT chr(floor(65 + random()*25)::int) || 
                                chr(floor(65 + random()*25)::int) || chr(floor(65 + random()*25)::int) || 
                                chr(floor(65 + random()*25)::int) || chr(floor(65 + random()*25)::int) || 
                                chr(floor(65 + random()*25)::int),
                ...
                self.connection.commit()
                self.cursor.execute(f"""INSERT INTO public.match
                                SELECT generate_series,
                                DateTime.toZone('UTC'),
                                floor(65 + random()*25)::int,
                                floor(65 + random()*25)::int
    ...
    def text_search(self, mode, word, table):
        ...
        try:
            if mode == '1':
                ...
                self.cursor.execute(
                    f'select * from public."{table}" where not (to_tsvector(coach_name) @@ to_tsquery(\'{self.words}\'))')
            elif mode == '2':
                self.words = word.split()
                self.words = '&'.join(self.words)
                self.cursor.execute(
                    f'select * from public."{table}" where (to_tsvector(coach_name) @@ to_tsquery(\'{self.words}\'))')
            else:
                return None
            self.rows = self.cursor.fetchall()
            self.columns = []
